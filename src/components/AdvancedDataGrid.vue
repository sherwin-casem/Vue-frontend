  ...
  <div class="advanced-data-grid">
    ...
    <v-card v-else class="data-table-card" elevation="2">
      <v-data-table
        ...
        @click:row="onRowClick"
        @update:selection="(val) => console.log('Selection changed:', val)"
      >
    ...
    <SelectionToolbar
      :selected-items="internalSelection"
      :loading="selectionToolbarLoading"
      :delete-loading="selectionToolbarLoading"
      :delete-progress="selectionDeleteProgress"
...
interface Props {
  ...
  loading?: boolean
  itemKey?: string
  tableKey?: string // For localStorage preferences
}
...
const selectionToolbarLoading = ref(false)
const selectionDeleteProgress = ref(0)
...
// Grouped table items (only actual data items, no group headers as rows)
const groupedTableItems = computed(() => {
  if (groupByFields.value.length === 0) {
    return filteredAndSortedItems.value
  }

  // Return only the actual data items, filtering out group headers
  return displayItems.value.filter((item) => !item._isGroupHeader)
})
...
const itemKeyField = computed(() => {
  ...
  const possibleKeys = ['id', 'site_id', 'charge_point_id', 'charging_profile_id']
...
const handleSelectionExport = (format: 'pdf' | 'excel' | 'csv') => {
  emit('export-selected', format, internalSelection.value)
}

const handleSelectionDelete = () => {
  emit('delete-selected', internalSelection.value)
}
...
watch(
  ...
  (newColumns) => {
    ...
    // Ensure columnOrder has all available columns (for new columns added)
...
.v-theme--dark .settings-btn {
  color: rgb(var(--v-theme-on-surface)) !important;
  border-color: rgba(255, 255, 255, 0.3) !important;
  background: rgba(var(--v-theme-surface), 0.9) !important;
}

.v-theme--dark .settings-btn:hover {
  border-color: rgb(var(--v-theme-primary)) !important;
...
:deep(.v-data-table__wrapper tbody tr:has(.group-header-cell)) {
  background: rgba(var(--v-theme-primary), 0.04) !important;
}

:deep(.v-data-table__wrapper tbody tr:has(.group-header-cell):hover) {
  background: rgba(var(--v-theme-primary), 0.08) !important;
}

/* Group item row styling */
:deep(.v-data-table__wrapper tbody tr:has(.group-item-cell)) {
  border-left: 3px solid rgba(var(--v-theme-primary), 0.3);
}

:deep(.v-data-table__wrapper tbody tr:has(.group-item-cell) td) {
  border-left: 1px solid rgba(var(--v-theme-outline-variant), 0.5);
}
